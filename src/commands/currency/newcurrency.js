        ...
        try {
            if (args.length == 5) { 
                ...
                if (args[2] == "t" && !isNaN(args[3]) && !isNaN(args[4])) {
                    await sql.addCur(message.guild.id, args[0], args[1], 1, args[3], args[4])
                    ...
                    await sql.addCur(message.guild.id, args[0], args[1], 0, args[3], args[4])
                    message.channel.send(`Successfully created new currency named ${args[0]} with the symbol of ${args[1]}`)
                    logger.info(`${message.guild.id} has created a new currency named ${args[0]} with the symbol of ${args[1]}`)
                ...
                if (args[2] == "f") { 
                    await sql.addCur(message.guild.id, args[0], args[1], 0, null, null) 
                    message.channel.send(`Successfully created new currency named ${args[0]} with the symbol of ${args[1]}.`)
                    logger.info(`${message.guild.id} has created a new currency named ${args[0]} with the symbol of ${args[1]}`)
                } else {
            ...
            message.channel.send("An error has occured. Does currency already exist?")
            logger.error(`${message.guild.id} has attempted to create a new currency named ${args[0]} but an error has occured`)
        }
    }
}